 ...
 */
function sectionOutline(af, c0, c1, steps, roundLE) {
  const up = [];
  const lo = [];
 ...
 */
function buildLiftingSurface(o) {
 ...
 */
function buildFuselage(table, ringZ, segs, windows) {
 ...
 */
function sweep(pts, ra, rb, sides = 10, ref = new THREE.Vector3(0, 0, 1)) {
  const pos = [];
  const uv = [];
  const idx = [];
...
function drawRivets(ctx, x0, y0, x1, y1, colour, pitch, dot) {
